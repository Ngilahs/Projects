...
Public Class frmcopyofReceipt
    ...
    Private Sub LoadItemstoDatagrid()
        Try
            sql = "Select  SD.ItemBarcode, P.Description, CONCAT(SD.TransQTY,' ','X') AS TransQTY, P.Selling_Price, CONCAT(SD.Subtotal,' ',SD.VatCode) AS Subtotal FROM sales_details SD inner join product P on (P.ProductNo = SD.ItemBarcode) WHERE TransID = '" & frmListofmaterials.dgw.CurrentRow.Cells(0).Value & "'"
            ConnDB()
            ...
            Do While dr.Read = True
                'CONCAT(SD.ItemBarcode,'',P.Description) AS Item
                ListBox1.Items.Add(dr("ItemBarcode"))
                ListBox1.Items.Add(dr("Description"))
                ListBox2.Items.Add(dr("TransQTY"))
                ListBox2.Items.Add((""))
                ListBox3.Items.Add(dr("Selling_Price"))
                ListBox3.Items.Add(dr("Subtotal"))
                'ListBox4.Items.Add(dr("Subtotal"))
                'ListBox4.Items.Add((""))
                y += 28
            Loop
            ListBox1.Height += y
            ...
            ListBox3.Height += y
            'ListBox4.Height += y
            Panel2.Location = New Point(Me.Panel2.Location.X, Me.Panel2.Location.Y + y)
            Panel1.Height += y
            'Panel2.Height += y
        Finally
            cmd.Dispose()
    ...
    Private Sub loadcompanyinfo()
        Try
            ...
            If dr.Read = True Then
                ...
                Label2.Text = dr("Address")
                LblMobileNo.Text = dr("TelephoneNo")
                lblMpesaTillNo.Text = dr("MpesaTillNo")
                lblPin.Text = dr("PINNo")
            End If
    ...
    Private Sub LoadReceiptInfo()
        Try
            sql = "SELECT TransID, TOTALPRICE, TENDEREDAMOUNT, CHANGEAMOUNT, VATABLEAMT, VAT, ITEMCOUNT, RECEIPTNO, PaymentOption, Customer, SessIDNo FROM s_payment WHERE TransID = '" & frmListofmaterials.dgw.CurrentRow.Cells(0).Value & "'"
            ConnDB()
            cmd = New MySqlCommand(sql, conn)
            dr = cmd.ExecuteReader()
            If dr.Read = True Then
                LblTransNo.Text = dr("TransID")
                lblSubtotal.Text = dr("TOTALPRICE")
                LblDate.Text = Date.Now.ToString("MM/dd/yyyy")
                LblTime.Text = Date.Now.ToString("hh:mm:ss")
                lblSubtotal.Text = FormatNumber(dr("TOTALPRICE"))
                lblTendered.Text = FormatNumber(dr("TENDEREDAMOUNT"))
                lblChange.Text = FormatNumber(dr("CHANGEAMOUNT"))
                lblTotalItems.Text = dr("ITEMCOUNT")
                'lblcashgiven.Text = FormatNumber(dr("TENDEREDAMOUNT"))
                Vatableamt.Text = FormatNumber(dr("VATABLEAMT"))
                lblVATtot.Text = FormatNumber(dr("VAT"))
                lblReceiptNo.Text = (dr("RECEIPTNO"))
                Label22.Text = FrmMain.tsslUser.Text
                LblSaletype.Text = dr("PaymentOption")
                LBLCustomer.Text = dr("Customer")
                LBLTillNo.Text = FrmMain.LBLComputerName.Text
    ...
    Public Sub PrintHeader()
        Try
            ...
            If dr.Read = True Then
                ...
                'TextToPrint &= spcLen5 & StringToPrint & Environment.NewLine
                TextToPrint &= "Trans#: " & frmListofmaterials.dgw.CurrentRow.Cells(0).Value & " " & "Recpt#: " & frmListofmaterials.dgw.CurrentRow.Cells(1).Value & Environment.NewLine
    ...
    Public Sub ItemsToBePrinted()
        ...
        'cont.Open()
        Try
            sql = "Select  SD.ItemBarcode, P.Description, SD.TransQTY, P.Selling_Price, CONCAT(SD.Subtotal,' ',SD.VatCode) AS Subtotal FROM sales_details SD inner join product P on (P.ProductNo = SD.ItemBarcode) WHERE TransID = '" & frmListofmaterials.dgw.CurrentRow.Cells(0).Value & "'"
    ...
    Public Sub printFooter()
        ...
        Try
            ...
            If dr.Read = True Then
                ...
                TextToPrint &= "THIS IS COPY OF LEGAL RECEIPT" & Environment.NewLine
    ...
    Private Sub BTNReprintReceipt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNReprintReceipt.Click
        PrintDocument1.PrinterSettings.PrinterName = "E-PoS 76mm Impact Dot Matrix Printer"
        PrintHeader()
        ItemsToBePrinted()
        printFooter()
        PrintDocument1.Print()
